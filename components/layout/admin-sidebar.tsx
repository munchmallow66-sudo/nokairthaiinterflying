...
export function AdminSidebar() {
  ...
  return (
    <>
      ...
      <header className="lg:hidden sticky top-0 z-40 bg-slate-950/95 border-b border-slate-800/80 px-4 py-3 flex items-center justify-between backdrop-blur-xl w-full">
        <Link href="/admin" className="block">
          <BrandLogo size="sm" variant="light" />
        </Link>
        <div className="flex items-center space-x-2">
          <LanguageSwitcher variant="minimal" className="scale-85" />
          <button
          ...
          >
            {isMobileOpen ? <X className="h-5 w-5 text-tif-gold" /> : <Menu className="h-5 w-5" />}
          </button>
        </div>
      </header>
      ...
      {/* Mobile Drawer Panel */}
      {isMobileOpen && (
        <div className="lg:hidden fixed top-[57px] inset-x-0 z-50 bg-slate-950 border-b border-slate-800 p-4 space-y-4 shadow-2xl animate-in slide-in-from-top-2">
          <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 font-display px-2">
            {t("commandCenter")}
          </div>
              ...
              return (
                <Link
                  ...
                  className={cn(
                    ...
                    isActive
                      ? "bg-slate-900 text-tif-gold border border-tif-gold/30"
                      : "text-slate-400 hover:bg-slate-900/60 hover:text-slate-200"
                  )}
                >
                  <div className="flex items-center space-x-3">
                    <Icon className={cn("h-4 w-4", isActive ? "text-tif-gold" : "text-slate-400")} />
                    <span>{item.name}</span>
                  </div>
                  {item.badge && (
          ...
          <div className="pt-3 border-t border-slate-800/80 flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <div className="h-8 w-8 rounded-xl bg-tif-gold/10 border border-tif-gold/30 text-tif-gold flex items-center justify-center font-bold text-xs">
                {user?.name ? user.name.slice(0, 2).toUpperCase() : "AD"}
              </div>
              <div className="truncate max-w-[180px]">
                <p className="text-xs font-semibold text-white truncate">{user?.name || "Officer"}</p>
                <p className="text-[10px] text-slate-400 truncate font-mono">{user?.email || "admin@tif.ac.th"}</p>
              </div>
            </div>
            <button
      ...
      <aside className="hidden lg:flex w-64 bg-slate-950/90 text-slate-300 min-h-screen border-r border-slate-800/80 flex-col justify-between shrink-0 backdrop-blur-xl relative z-20">
        <div>
          {/* Header */}
